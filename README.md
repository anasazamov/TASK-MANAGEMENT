...
- Ikki joyda TypeSafe (Jev) qarori ishlatiladi. Jonli suhbatda har bir gap uchun «bu tizimga aytilganmi?» deb so‘raladi: yonidagi suhbat agentga yuborilmaydi, mikrofon esa ochiq qoladi. Bir xil ismli xodimlar chiqqanda buyruq mazmuni, lavozim va so‘rovchining bo‘limi bo‘yicha kimligi aniqlanadi; ishonch `TYPESAFE_PERSON_MIN` (0.85) dan past bo‘lsa, avvalgidek foydalanuvchidan so‘raladi. Model matn yozmaydi va amal bajarmaydi — faqat tanlov yoki ehtimollik qaytaradi. `TYPESAFE_API_KEY` bo‘lmasa yoki xizmat javob bermasa, ikkala joy ham eski holatiga qaytadi: hamma gap qabul qilinadi, ism so‘raladi.
...
- TTS ovozi `MUXLISA_SPEAKER` bilan tanlanadi: `0` — ayol, `1` — erkak. Ovoz katalogi yo‘q. Muxlisa bir so‘rovda 512 belgigacha matnni o‘qiydi. Hisob shu chegaradan oshsa, ovoz uchun qisqa variant olinadi, to‘liq matn ekranda qoladi. Chegara ovozli nusxaga qo‘llanadi: sana va sonlar so‘z bilan yozilgani uchun u ekrandagi matndan ancha uzun bo‘lishi mumkin.
