...
## Ovozli yordamchi: VoiceLab + OpenAI
...
# O‘zbekcha Gulnoza ovozi. Bo‘sh qoldirilsa ham Gulnoza ishlatiladi.
VOICELAB_VOICE_ID=voice_01J9NEUTRAL0000000000000001
```
...
Kalitlarni kiritgandan so‘ng `serve.py` jarayonini qayta ishga tushiring. Kalitlar Git’dan chiqarilgan `.env`da qoladi; HTML/JavaScript/API javoblarida qaytarilmaydi. VoiceLab kalitida oddiy audio uchun `speech_to_text: access`, `text_to_speech: access`, `voices: read`, jonli rejim uchun esa `stt:realtime` va `tts:realtime` ruxsatlari kerak. VoiceLab LLM ruxsati kerak emas. OpenAI kalitida tanlangan modeldan foydalanish huquqi va API balansi bo‘lishi kerak. `vlk_` bilan boshlangan VoiceLab kaliti OpenAI kaliti o‘rnida ishlamaydi va OpenAI serveriga yuborilmaydi.
...
- Brauzer o‘z Django serveriga WebSocket orqali ulanadi. Bir martalik 60 soniyalik ticket foydalanuvchi sessiyasi va Origin bilan bog‘langan. Django VoiceLab’ga server orqali ulanadi; VoiceLab kaliti va provider ticket brauzerga chiqmaydi.
- Audio avval Django xotirasida yig‘iladi va mahalliy ovoz tekshiruvidan o‘tadi. Faqat tasdiqlangan audio VoiceLab’ga yuboriladi. VoiceLab yakuniy transkriptni gap tugagach qaytaradi; oraliq so‘zlar ko‘rsatilmaydi. OpenAI funksiyalarni bajarib javob tayyorlaydi. TTS 24 kHz PCM bo‘laklari kelishi bilan o‘qiladi. Ovoz tekshiruvi, STT, LLM va tarmoq tezligi javob kechikishiga ta’sir qiladi.
- VoiceLab `realtime_stt_unavailable`, timeout yoki xizmatning vaqtinchalik xatosini qaytarsa, server xotiradagi ayni PCM’ni WAV qilib VoiceLab oddiy STT xizmatiga bir marta yuboradi. Qayta aytish talab qilinmaydi. Har gap uchun alohida idempotency kaliti ishlatiladi; foydalanuvchi uzilganda kutilayotgan so‘rov bekor qilinadi. Provayder allaqachon qabul qilgan audio so‘rovi uchun xarajat bo‘lishi mumkin. REST yo‘li VoiceLab tarixiga yozilishi mumkin. Sukut va kredit tugashi qayta pullik STT chaqirmaydi. Vaqtinchalik xatoda jonli suhbat qayta ulanadi; uchta ketma-ket xatodan keyin mikrofon o‘chiriladi. Kredit, ruxsat va limit xabarlari alohida ko‘rsatiladi.
...
- Yozuv 29 soniyada avtomatik to‘xtaydi (VoiceLab sinxron limiti 30 soniya). 10 MiB gacha WebM, WAV, MP3, M4A/AAC, OGG/Opus, FLAC faylini ham tanlash mumkin.
...
- TTS uchun VoiceLab Lison katalogidagi o‘zbekcha **Gulnoza** ovozi aniq ID bilan belgilangan; bo‘sh sozlamada ham shu ovoz ishlatiladi. Katalogning birinchi elementi avtomatik olinmaydi. `VOICELAB_VOICE_ID` bilan boshqa ovoz tanlash mumkin (Sardor: `voice_xdQMoRJkjHbPIB-jounXzKq8`). Katalog 5 daqiqa keshlanadi. VoiceLab’ning 1000 UTF-8 bayt chegarasidan uzun javob uchun qisqa ovozli variant olinadi, to‘liq matn ekranda qoladi.
...
- Jonli TTS birinchi audiodan oldin vaqtincha uzilsa, shu matn va ovoz bilan VoiceLab REST TTS bir marta sinab ko‘riladi. Bir xil imzoli javob uchun qayta urinish kaliti o‘zgarmaydi. Audio yuborilgach avtomatik takrorlash yo‘q; ruxsat, kredit, limit va format xatolari alohida ko‘rsatiladi. To‘xtatish zaxira so‘rovini ham bekor qiladi.
- Xom audio faqat VoiceLab’ga yuboriladi. OpenAI eshitilgan matn, suhbat konteksti va vositalar orqali foydalanuvchiga ruxsat etilgan topshiriq, tarix hamda xodim ma’lumotlarini oladi (`store=False`). TTS uchun agent javobining matni VoiceLab’ga yuboriladi. VoiceLab audio/transkripsiyani o‘z tarixida saqlashi mumkin.
