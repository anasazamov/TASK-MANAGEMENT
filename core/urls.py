from django.contrib.auth.views import LogoutView
from django.urls import path
from . import views, voice_views, agent_views, realtime

urlpatterns = [
    path('login/', views.SignInView.as_view(), name='login'),
    path('logout/', LogoutView.as_view(), name='logout'),
    path('account/password/', views.password_change, name='password_change'),
    path('', views.dashboard, name='dashboard'),
    path('tasks/', views.task_list, name='tasks'),
    path('tasks/new/', views.task_create, name='task_create'),
    path('voice/status/', voice_views.status, name='voice_status'),
    path('voice/transcribe/', voice_views.transcribe, name='voice_transcribe'),
    path('voice/draft/', voice_views.draft, name='voice_draft'),
    path('voice/speak/', voice_views.speak, name='voice_speak'),
    path('voice/realtime-session/', realtime.session, name='voice_realtime_session'),
    path('voice/realtime-speak/', realtime.speak, name='voice_realtime_speak'),
    path('agent/state/', agent_views.state, name='agent_state'),
    path('agent/message/', agent_views.message, name='agent_message'),
    path('agent/reset/', agent_views.reset, name='agent_reset'),
    path('tasks/<int:pk>/', views.task_detail, name='task_detail'),
    path('tasks/<int:pk>/action/', views.perform_action, name='task_action'),
    path('tasks/<int:pk>/participants/add/', views.add_participant, name='task_participant_add'),
    path('tasks/<int:pk>/files/add/', views.add_attachment, name='task_attachment_add'),
    path('tasks/<int:pk>/files/<int:attachment_id>/', views.download_attachment, name='task_attachment'),
    path('tasks/<int:pk>/files/<int:attachment_id>/delete/', views.delete_attachment, name='task_attachment_delete'),
    path('tasks/<int:pk>/participants/<int:participant_id>/', views.participant_action, name='task_participant_action'),
    path('employees/', views.employees, name='employees'),
    path('pages/', views.generated_pages, name='generated_pages'),
    path('pages/<int:pk>/', views.generated_page, name='generated_page'),
    path('pages/<int:pk>/content/', views.generated_page_content, name='generated_page_content'),
    path('pages/<int:pk>/delete/', views.generated_page_delete, name='generated_page_delete'),
    path('structure/', views.structure, name='structure'),
    path('employees/new/', views.employee_edit, name='employee_create'),
    path('employees/<int:pk>/edit/', views.employee_edit, name='employee_edit'),
    path('employees/<int:pk>/toggle/', views.employee_toggle, name='employee_toggle'),
    path('employees/<int:pk>/password/', views.employee_password, name='employee_password'),
    path('healthz/', views.healthz, name='healthz'),
    path('telegram/webhook/<str:secret>/', views.telegram_webhook, name='telegram_webhook'),
    path('telegram/login/', views.telegram_login, name='telegram_login'),
    path('push-worker.js', views.push_worker, name='push_worker'),
    path('push/subscribe/', views.push_subscribe, name='push_subscribe'),
    path('push/unsubscribe/', views.push_unsubscribe, name='push_unsubscribe'),
    path('push/test/', views.push_test, name='push_test'),
    path('notifications/', views.notifications, name='notifications'),
    path('notifications/live/', views.notifications_live, name='notifications_live'),
    path('notifications/read/', views.notifications_read, name='notifications_read'),
    path('timeline/', views.timeline, name='timeline'),
    path('chains/', views.chains, name='chains'),
]
