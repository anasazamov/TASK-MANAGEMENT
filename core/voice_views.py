...
@require_GET
def status(request):
    llm, speech = voice.configured(), bool(settings.MUXLISA_API_KEY)
...
def transcribe(request):
    ...
    if length > settings.VOICE_MAX_AUDIO_BYTES + 65536:
        raise voice.VoiceError('audio_too_large', 'Audio hajmi 5 MB dan oshmasligi kerak.', 413)
    upload = request.FILES.get('audio')
    if not upload or len(request.FILES) != 1:
        raise voice.VoiceError('audio_required', 'Bitta audio yozuv yuboring.')
    upload.seek(0)
    return JsonResponse(muxlisa.transcribe(upload))
...
def draft(request):
    ...
    speech = muxlisa.speech_excerpt(spoken)
...
def speak(request):
    ...
    audio = muxlisa.synthesize(data['text'])
